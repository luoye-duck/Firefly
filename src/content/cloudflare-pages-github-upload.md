...
Cloudflare Pages 直接上传方式对单文件大小和总项目体积都有一定限制。当你的网站包含较多资源（图片、视频、字体等），就很容易触发这个限制。

**解决方案：** 把文件先上传到 GitHub 仓库，然后让 Cloudflare 连接你的 GitHub 仓库进行自动构建部署，完美绕过文件大小限制！
...
🔗 链接直达：[前往 GitHub 注册](https://github.com/signup)
...
> 💡 **小提示：** GitHub 是全球最大的代码托管平台，免费版就有足够的存储空间，完全够个人建站使用。

---

## 第二步：创建 GitHub 仓库

注册好账号后，我们需要创建一个仓库来存放网站文件。
...
2. 填写仓库信息：
   ...
   - **Public / Private**：选择 **Public**（公开），这样 Cloudflare 才能免费访问
...
3. 将你网站的所有文件**拖拽**到上传区域
   - ⚠️ **注意：** 如果网站主页面是 HTML 文件，请确保它命名为 `index.html`
   - 建议先在本地把文件整理好，确保目录结构清晰
4. 在下方填写提交信息（Commit changes），随便写个说明，比如 "上传网站文件"
5. 点击 **Commit changes** 提交

### 方式二：使用 Git 命令行上传（进阶）

如果你会基本的命令行操作，可以用 Git 上传：
...
| 对比 | Cloudflare 直接上传 | GitHub + Cloudflare |
|---|---|---|
| 文件大小限制 | 有，容易超限 | 基本无限制 |
| 更新方式 | 每次手动重新上传 | 自动部署 |
| 版本管理 | 无 | 有，可回退历史版本 |
...
**一句话总结：** 遇到 Cloudflare Pages 文件过大的问题，别纠结，直接把文件传到 GitHub，让 Cloudflare 连接 GitHub 自动构建部署，一劳永逸！
